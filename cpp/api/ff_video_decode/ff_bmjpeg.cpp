 ...
 *     ffmpeg_jpeg.c
 * Desc:
 *     an example of ffmpeg read from memory
 * Author:
 *     wei.cheng
 *     shulin.shang
 */
...
int main(int argc, char* argv[])
{
    ...
    int ret = 0;
    int loop = 0;
    string output_file_name = "rencode-output";
...
#ifdef BM_PCIE_MODE
    if (argc != 4) {
        cerr << "Usage:"<< argv[0] << "sophon_idx:[0-63] zero_copy:[0-1] xxx.jpg" << endl;
#else
    if (argc != 2) {
    ...
    {
        cerr << "sophon_idx:" << sophon_idx << "Error, must be 0-63" << endl;
        return 0;
    }
    ...
    {
        cerr << "zero_copy:" << zero_copy << "Error, must be 0 or 1" << endl;
        return 0;
    }
#endif
...
#undef JPU_PAGE_UNIT_SIZE
    ...
    while (av_read_frame(pFormatCtx, &pkt) >= 0) {
        ...
        if (got_picture) {
            //save YUV data
            ret = saveYUV(pFrame, loop, output_file_name.c_str());
            if (ret < 0) {
                break;
            }
            ...
            ret = writeJPEG(pFrame, loop, output_file_name.c_str());
            if (ret < 0) {
                break;
            }
