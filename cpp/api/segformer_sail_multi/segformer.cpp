...
int SegFormer::pre_process(sail::BMImage &input, sail::BMImage &output)
{
  ...
  bmcv->convert_format(convert_img, output);

  std::cout << "convert_format convert_img to  output" << std::endl;
...
void SegFormer::SendInputTensor(sail::BMImage &convert_img)
{

  bmcv->bm_image_to_tensor(convert_img, *input_tensor.get());
}

int SegFormer::post_process(cv::Mat &color_seg_mat)
{
  ...
  for (int batch_idx = 0; batch_idx < 1; ++batch_idx)
  {
    ...
    cv::Mat new_color_seg_mat(color_seg.size(), color_seg[0].size(), CV_8UC3);
    //  color_seg_mat
    for (int i = 0; i < color_seg.size(); ++i)
    {
      for (int j = 0; j < color_seg[i].size(); ++j)
      {
        ...
        cv::Vec3b &pixel = new_color_seg_mat.at<cv::Vec3b>(i, j);
        pixel[0] = (color1[0]); // R
        pixel[1] = (color1[1]); // G
        pixel[2] = (color1[2]); // B
      }
    }
    color_seg_mat=new_color_seg_mat;
