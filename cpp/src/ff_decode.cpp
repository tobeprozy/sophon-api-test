...
//===----------------------------------------------------------------------===//
/*
 * This is a wrapper header of BMruntime & BMCV, aiming to simplify user's
 * program.
 */
...
bm_status_t avframe_to_bm_image(bm_handle_t& handle, AVFrame* in, bm_image* out,
                                bool is_jpeg, bool data_on_device_mem) {
  int plane = 0;
  ...
  switch (in->format) {
    ...
    default:
      printf(
          ...
          "yuv420,yuvj420,yuv422,yuvj422,yuv444,yuvj444,nv12,nv16,gray,rgb_"
          "packed,bgr_packed supported\n");
      break;
  }
  ...
  if (in->channel_layout == 101) { /* COMPRESSED NV12 FORMAT */
    ...
    if (mem_flags == USEING_MEM_HEAP1 &&
        bm_image_alloc_dev_mem_heap_mask(*out, mem_flags) != BM_SUCCESS) {
...
bm_status_t picDec(bm_handle_t& handle, const char* path, bm_image& img) {
  ...
  if (infile == nullptr) {
    av_log(NULL, AV_LOG_ERROR,
           "Open %s failed, please check if you have enough rights.\n", path);
  ...
  if (type < 0) {
    av_free(buffer);
    av_log(NULL, AV_LOG_ERROR,
           "ff_decode only support jpg, png, bmp. unknown pic signature.\n");
...
bm_status_t miscDec(bm_handle_t& handle, uint8_t* bs_buffer, int numBytes,
                    int type, bm_image& img) {
  bm_status_t ret;
  const AVCodec* codec;
...
bm_status_t jpgDec(bm_handle_t& handle, uint8_t* bs_buffer, int numBytes,
                   bm_image& img) {
...
#undef JPU_PAGE_UNIT_SIZE
  ...
  if (AV_PIX_FMT_YUVJ422P == pFrame->format) {
    ...
    I420Frame->data[1] = (uint8_t*)malloc(pFrame->linesize[1] *
                                          FF_ALIGN(I420Frame->height, 2) / 2);
    I420Frame->data[2] = (uint8_t*)malloc(pFrame->linesize[2] *
                                          FF_ALIGN(I420Frame->height, 2) / 2);
    libyuv::I422ToI420(
