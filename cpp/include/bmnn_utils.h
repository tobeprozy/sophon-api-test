 ...
 *      2. Any member in m_tensor has device memory like
 * bm_tensor_t\bm_image\bm_device_mem_t must be freed outside.
 */
class BMNNTensor {
  /**
 ...
 *      2. Device memory of input tensors must be provided outside, and will not
 * be freed here.
 ...
 public:
  ...
  BMNNNetwork(void* bmrt, const std::string& name) : m_bmrt(bmrt) {
    ...
    for (int i = 0; i < m_netinfo->output_num; ++i) {
      ...
      auto ret = bm_malloc_device_byte(m_handle, &m_outputTensors[i].device_mem,
                                       max_size);
      assert(BM_SUCCESS == ret);
    }
  ...
  int forward() {
    ...
    bool ok = bmrt_launch_tensor_ex(m_bmrt, m_netinfo->name, m_inputTensors,
                                    m_netinfo->input_num, m_outputTensors,
